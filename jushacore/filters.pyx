...
A filter function is a function on the data set, :math:`f\:{:}\;X\to \mathbb{R}^k`. The jushacore algorithm supports general, vector-valued functions, while the GUI is restricted to real-valued functions (the case :math:`k=1`) for simplicity.
...
Filter functions in Python jushacore
---------------------------------

A number of one-dimensional filter functions is provided in the module ``jushacore.filters``.
...
from jushacore import n_obs, cmappertoolserror
