...
Drawing routines for jushacore output.
'''
import numpy as np
...
if sys.hexversion < 0x03000000:
    ...
    range = xrange
from jushacore.tools import qhull, shortest_path, pdfwriter, graphviz_node_pos, dict_values
...
if mplversion < [1, 0]:
    print ('Warning: Best results are obtained with Matplotlib version at '
           'least 1.0.\n'
           'Python jushacore should work with Matplotlib 0.99, too, but some '
           'features are\nmissing.')
if mplversion < [1, 1]:
...
#
# This will simplify and increase efficiency of the GUI's jushacoreOutputFrame.
...
def draw_2D(M, ax=None, node_labels=None, node_color=None, legend=True, verbose=False, node_scale=1000., rotate=True, minsizes=(), node_labels_scheme=None, node_color_scheme=None):
    '''
    Draw a jushacore output in 2D. The jushacore output may have higher-dimensional simplices.
    ...
    intersection?
    @todo: Rename this module as soon as the name L{draw_jushacore_output} becomes free.

    @param M: jushacore output
    @type M: L{jushacore_output}
    ...
    if S.dimension < 0:
        ...
        vertex_sizes = np.array([M.node_sizes[v] for v in vertices],
                                dtype=np.int)
        # Formula for the circle sizes as in Gurjeet Singh's jushacore code
...
def enclosing_rect(ch):
    ...
    raise RuntimeError('Algorithm did not converge. ' \
                           'Please file a bug report to jushacore team.')
...
def draw_3D(M, verbose=False):
    '''
    Draw a jushacore output in 3D. The jushacore output may have higher-dimensional simplices.
    ...
    from jushacore_output import simpl_complex
    ...
    # Formula for the nodes sizes as in Gurjeet Singh's jushacore code
...
def save_scale_graph_as_pdf(sgd, filename, log_yaxis=False, maxvertices=None,
    ...
    from jushacore import __version__
    pdfpage = pdfwriter.OnePagePdf(filename_no_ending + '.pdf',
                         title='Scale graph',
                         compress=True,
                         height=Height, width=Width,
                         creator='Python jushacore ' + __version__)
...
def node_color_from_point_color(M, point_color, inverse_labels):
    ...
    Only jushacore output nodes which correspond to simplices are processed.
